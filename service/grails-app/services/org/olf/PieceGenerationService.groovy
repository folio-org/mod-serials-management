  ...
  public createPiecesTransient (SerialRuleset ruleset, LocalDate startDate) {
    ...
    if (!!ruleset?.omission) {
      ...
      for (Integer i = 0; i < ruleset?.omission?.rules?.size(); i++) {
        ...
        String formattedOmissionPatternType = RGX_PATTERN_TYPE.matcher(ruleset?.omission?.rules[i]?.patternType?.value).replaceAll { match -> match.group(1).toUpperCase() }
        ...
        for (Integer j = 0; j < dates.size(); j++) {
          if(opc.compareDate(ruleset?.omission?.rules[i], dates[j]?.date, j, dates)){
            dates[j].omitted = true
          }
        }
      }
    }
